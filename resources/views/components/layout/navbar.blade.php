...
@php
    $hasDarkTop = request()->routeIs('home') || request()->routeIs('divisions.show') || $fullpage;
    $links = [
        ...
        ['name' => 'Tim Lomba', 'route' => 'teams.index', 'active' => request()->routeIs('teams.*')],
...
<nav x-data="{ mobileMenuOpen: false, scrolled: false }"
     ...
     class="fixed top-0 inset-x-0 z-50 transition-all duration-300 backdrop-blur-md border-b border-white/15"
     :class="{
         'bg-slate-950/70 shadow-lg': scrolled,
         'bg-black/25': !scrolled
     }">
        ...
        <div class="flex items-center justify-between h-20">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('home') }}" class="flex items-center space-x-3">
                    <img src="{{ asset('images/logo-aptrg.svg') }}" alt="APTRG Logo" class="h-12 w-12 rounded-full object-contain">
                    <div>
                        <span class="block text-lg font-bold tracking-tight text-white transition-colors">
                            APTRG
                        </span>
                        <span class="block text-xs font-medium text-white/80 transition-colors">
                            Telkom University
                        </span>
                    </div>
                </a>
            ...
            <div class="hidden md:flex md:items-center md:space-x-8">
                @foreach ($links as $link)
                    <a href="{{ route($link['route']) }}"
                       class="inline-flex items-center h-20 px-1 text-sm font-semibold transition-colors border-b-4 {{ $link['active'] ? 'border-primary text-primary' : 'border-transparent text-white hover:text-primary hover:border-primary/40' }}">
                        {{ $link['name'] }}
                    </a>
                @endforeach
            </div>
            ...
            <div class="flex items-center md:hidden">
                <button type="button" 
                        ...
                        class="p-2 rounded-lg focus:outline-none transition-colors text-white hover:text-primary">
