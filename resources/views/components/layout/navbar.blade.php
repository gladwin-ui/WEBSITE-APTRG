<nav class="sticky top-0 z-50 bg-surface border-b border-line shadow-sm" x-data="{ mobileMenuOpen: false }">
        ...
        <div class="flex items-center justify-between h-20">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('home') }}" class="flex items-center space-x-3">
                    ...
                    <div>
                        <span class="block text-lg font-bold tracking-tight text-ink">APTRG</span>
                        <span class="block text-xs font-medium text-body">Telkom University</span>
                    </div>
                </a>
            </div>
            ...
            <div class="hidden md:flex md:items-center md:space-x-8">
                ...
                @foreach ($links as $link)
                    <a href="{{ route($link['route']) }}"
                       class="inline-flex items-center h-20 px-1 text-sm font-semibold transition-colors border-b-4 {{ $link['active'] ? 'border-primary text-primary' : 'border-transparent text-ink hover:text-primary hover:border-primary/40' }}">
                        {{ $link['name'] }}
                    </a>
                @endforeach
            </div>
            ...
            <div class="flex items-center md:hidden">
                <button type="button" 
                        ...
                        class="p-2 text-ink hover:text-primary rounded-lg focus:outline-none">
